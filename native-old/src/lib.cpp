    ...
    jmethodID method)
{
    ...
    if (jni_env->IsSameObject(classLoaderZ, classLoaderX))
        return;
    if (extClassLoader != null && jni_env->IsSameObject(classLoaderZ, extClassLoader))
        return;
                          ...
                          jvalue return_value)
{
    ...
    if (invokingInfo->hooks == null)
        return;
    if (*((void **)(invokingInfo->targetMethod)) != *((void **)method))
        return;
    ...
    jclass klass)
{
    ...
    if (klassLoader == null)
        return;
    if (jni_env->IsSameObject(klassLoader, classLoaderX))
        return;
    if (jni_env->IsSameObject(klassLoader, extClassLoader))
        return;
...
void initNative(JNIEnv *env, jclass owner, bool isCompat = false)
{
    std::cout << "initNative called" << std::endl;

    std::cout << "is Compat: " << isCompat << std::endl;
    ...
    std::cout << "before do sth for fun" << std::endl;

    if (isCompat)
    {
        std::cout << "fuck you karlatemp" << std::endl;
        exit(0);
    }
    else
    {
...
JNIEXPORT jobject JNICALL Java_io_github_karlatemp_jvmhook_core_Bootstrap_valueOfPointer(JNIEnv *, jclass, jlong pointer, jint index)
{
...
JNIEXPORT void JNICALL Java_io_github_karlatemp_jvmhook_core_Bootstrap_unregisterHook(JNIEnv *env, jclass, jclass target, jstring metName, jstring metDesc, jobject hook)
{
    auto m = findMetOfHook(env, target, metName, metDesc);
    if (m == null)
        return;
...
extern "C" JNIEXPORT jboolean JNICALL Java_me_earzuchan_sakiko_core_KarlatempNativeCompat_initNative(JNIEnv *env, jclass owner)
{
    initNative(env, owner, true);
